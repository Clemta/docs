...
1. **API Key-based Rate Limiting**: Applied to the `/company/initiate` endpoint
2. **Company ID-based Rate Limiting**: Applied to the `/company/status/{company_id}` endpoint
...
### For `/company/status/{company_id}` Endpoint
...
### Successful Request to `/company/initiate`
...
### Rate Limited Request to `/company/status/{company_id}`
