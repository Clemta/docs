...
1. **API Key-based Rate Limiting**: Applied to the `/formation/initiate` endpoint
2. **Company ID-based Rate Limiting**: Applied to the `/formation/status/{company_id}` endpoint
...
<ResponseField name="X-RateLimit-Status-Endpoint" type="string">
  Shows the rate limit for status endpoint per company ID (e.g., "60 requests per minute per company ID")
</ResponseField>
...
### For `/formation/initiate` Endpoint
...
### For `/formation/status/{company_id}` Endpoint
...
<Steps>
  <Step title="Implement exponential backoff">
    When receiving a 429 response, wait the suggested time in the `Retry-After` header and then retry with increasing delays.
  </Step>
...
### Successful Request to `/formation/initiate`
...
### Rate Limited Request to `/formation/status/{company_id}`
